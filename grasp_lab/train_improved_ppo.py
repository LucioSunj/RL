"""
改进的PPO配置 - 专为抓取任务优化
主要改进：
1. 更适合的超参数设置
2. 学习率调度优化
3. 网络架构简化
4. 训练稳定性增强
"""

import os
import sys
import time
import argparse
import numpy as np
import torch
import wandb
from datetime import datetime
from typing import Dict, List
import json

# 添加路径
manipulator_grasp_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'manipulator_grasp')
if manipulator_grasp_path not in sys.path:
    sys.path.insert(0, manipulator_grasp_path)

from improved_grasp_env import ImprovedGraspEnv
from ppo_agent import PPOAgent
from video_recorder import create_evaluation_video_manager


class ImprovedPPOTrainer:
    """改进的PPO训练器 - 专为抓取任务优化"""
    
    def __init__(self, config: Dict):
        self.config = config
        
        # 创建改进的环境
        self.env = ImprovedGraspEnv(
            robot_type=config['robot_type'],
            max_episode_steps=config['max_episode_steps'],
            use_image_obs=config['use_image_obs'],
            difficulty=config['difficulty'],
            headless=config['headless']
        )
        
        # 创建agent - 使用优化的超参数
        self.agent = PPOAgent(
            observation_space=self.env.observation_space,
            action_space=self.env.action_space,
            lr_actor=config['lr_actor'],
            lr_critic=config['lr_critic'],
            gamma=config['gamma'],
            gae_lambda=config['gae_lambda'],
            clip_epsilon=config['clip_epsilon'],
            entropy_coef=config['entropy_coef'],
            value_loss_coef=config['value_loss_coef'],
            max_grad_norm=config['max_grad_norm'],
            target_kl=config['target_kl'],
            n_epochs=config['n_epochs'],
            batch_size=config['batch_size'],
            buffer_size=config['buffer_size'],
            device=config['device']
        )
        
        # 训练统计
        self.episode_rewards = []
        self.episode_lengths = []
        self.success_rates = []
        self.phase_progress = []
        self.best_reward = float('-inf')
        self.best_success_rate = 0.0
        
        # 文件路径
        self.checkpoint_dir = config['checkpoint_dir']
        os.makedirs(self.checkpoint_dir, exist_ok=True)
        
        # 成功episodes记录
        self.recent_successes = []
        self.recent_phases = []
        
        # 视频管理器
        self.video_manager = create_evaluation_video_manager(config)
        print(f"Video recording: every {self.video_manager.video_interval} evaluations")
        
    def train(self):
        """主训练循环"""
        print("Starting Improved PPO training for grasping...")
        print(f"Environment: {type(self.env).__name__}")
        print(f"Difficulty: {self.env.difficulty}")
        print(f"Observation space: {self.env.observation_space}")
        print(f"Action space: {self.env.action_space}")
        print(f"Device: {self.agent.device}")
        
        start_time = time.time()
        global_step = 0
        episode = 0
        
        # 训练日志
        print("\n" + "="*80)
        print("Training Progress:")
        print("="*80)
        
        while global_step < self.config['total_timesteps']:
            # 收集经验
            episode_reward, episode_length, episode_info = self._collect_rollout()
            
            episode += 1
            global_step = self.agent.total_steps
            
            # 记录episode统计
            self.episode_rewards.append(episode_reward)
            self.episode_lengths.append(episode_length)
            
            # 记录成功率和阶段进度
            episode_success = episode_info.get('success', False)
            max_phase = episode_info.get('max_phase', 0)
            
            self.recent_successes.append(episode_success)
            self.recent_phases.append(max_phase)
            
            # 保持最近100个episodes的记录
            if len(self.recent_successes) > 100:
                self.recent_successes.pop(0)
                self.recent_phases.pop(0)
            
            current_success_rate = np.mean(self.recent_successes)
            avg_phase_progress = np.mean(self.recent_phases)
            self.success_rates.append(current_success_rate)
            self.phase_progress.append(avg_phase_progress)
            
            # 更新网络
            if self.agent.buffer.size >= self.config['buffer_size']:
                obs, _ = self.env.reset()
                update_stats = self.agent.update(obs)
                
                # 记录训练统计
                log_data = {
                    'episode': episode,
                    'global_step': global_step,
                    'episode_reward': episode_reward,
                    'episode_length': episode_length,
                    'episode_success': episode_success,
                    'max_phase_reached': max_phase,
                    'success_rate_100': current_success_rate,
                    'avg_phase_progress_100': avg_phase_progress,
                    'mean_reward_100': np.mean(self.episode_rewards[-100:]),
                    'std_reward_100': np.std(self.episode_rewards[-100:]),
                    **update_stats
                }
                
                # wandb记录
                if self.config['use_wandb']:
                    wandb.log(log_data, step=global_step)
                
                # 控制台输出
                if episode % self.config['log_interval'] == 0:
                    elapsed_time = time.time() - start_time
                    fps = global_step / elapsed_time
                    
                    print(f"\nEpisode: {episode:6d} | Step: {global_step:8d}")
                    print(f"Reward: {episode_reward:8.2f} | Length: {episode_length:4d} | Success: {episode_success}")
                    print(f"Success Rate: {current_success_rate:6.3f} | Avg Phase: {avg_phase_progress:.2f}")
                    print(f"Mean Reward: {log_data['mean_reward_100']:8.2f} | Std: {log_data['std_reward_100']:6.2f}")
                    print(f"Policy Loss: {update_stats['policy_loss']:8.4f} | Value Loss: {update_stats['value_loss']:8.4f}")
                    print(f"Entropy: {update_stats['entropy']:8.4f} | KL Div: {update_stats['kl_divergence']:8.4f}")
                    print(f"FPS: {fps:.1f} | Time: {elapsed_time/60:.1f}m")
                    print("-" * 80)
                
                # 保存检查点
                if episode % self.config['save_interval'] == 0:
                    self._save_checkpoint(episode, global_step)
                
                # 评估
                if episode % self.config['eval_interval'] == 0:
                    eval_reward, eval_success_rate, eval_avg_phase = self._evaluate_with_video()
                    
                    eval_log = {
                        'eval_reward': eval_reward,
                        'eval_success_rate': eval_success_rate,
                        'eval_avg_phase': eval_avg_phase,
                        'eval_episode': episode
                    }
                    
                    if self.config['use_wandb']:
                        wandb.log(eval_log, step=global_step)
                    
                    print(f"\n{'='*50}")
                    print(f"EVALUATION - Episode {episode}")
                    print(f"Eval Reward: {eval_reward:.2f} | Success Rate: {eval_success_rate:.3f}")
                    print(f"Avg Phase Progress: {eval_avg_phase:.2f}")
                    print(f"{'='*50}\n")
                    
                    # 保存最佳模型
                    if eval_success_rate > self.best_success_rate:
                        self.best_success_rate = eval_success_rate
                        self.best_reward = eval_reward
                        self._save_best_model(episode, global_step, eval_reward, eval_success_rate)
        
        # 训练完成
        print(f"\nTraining completed!")
        print(f"Total episodes: {episode}")
        print(f"Total timesteps: {global_step}")
        print(f"Best success rate: {self.best_success_rate:.3f}")
        print(f"Best reward: {self.best_reward:.2f}")
        
        # 最终保存
        self._save_checkpoint(episode, global_step, final=True)
        
        # 关闭环境
        self.env.close()
    
    def _collect_rollout(self):
        """收集一个episode的经验"""
        obs, info = self.env.reset()
        episode_reward = 0
        episode_length = 0
        done = False
        
        max_phase_reached = 0
        
        while not done and episode_length < self.config['max_episode_steps']:
            # 获取动作
            action, log_prob, value = self.agent.get_action(obs)
            
            # 执行动作
            next_obs, reward, terminated, truncated, info = self.env.step(action[0])
            done = terminated or truncated
            
            # 记录最大阶段
            max_phase_reached = max(max_phase_reached, info.get('phase', 0))
            
            # 存储经验
            self.agent.store_transition(obs, action[0], reward, value, log_prob, done)
            
            # 更新
            obs = next_obs
            episode_reward += reward
            episode_length += 1
        
        # 返回episode信息
        episode_info = {
            'success': info.get('success', False),
            'max_phase': max_phase_reached,
            'final_phase': info.get('phase', 0),
            'object_grasped': info.get('object_grasped', False),
        }
        
        return episode_reward, episode_length, episode_info
    
    def _evaluate_with_video(self, n_episodes: int = 10):
        """评估当前策略 - 带视频录制功能"""
        print("Evaluating...")
        
        # 检查是否需要录制视频
        should_record = self.video_manager.start_evaluation()
        
        rewards = []
        successes = []
        phase_progresses = []
        
        for episode_idx in range(n_episodes):
            # 检查是否需要为这个episode录制视频
            recording_this_episode = should_record and self.video_manager.start_episode_recording(episode_idx)
            
            obs, info = self.env.reset()
            episode_reward = 0
            done = False
            steps = 0
            max_phase = 0
            
            while not done and steps < self.config['max_episode_steps']:
                # 确定性动作
                action, _, _ = self.agent.get_action(obs, deterministic=True)
                obs, reward, terminated, truncated, info = self.env.step(action[0])
                done = terminated or truncated
                episode_reward += reward
                steps += 1
                max_phase = max(max_phase, info.get('phase', 0))
                
                # 录制视频帧
                if recording_this_episode:
                    try:
                        # 获取渲染图像
                        render_output = self.env.render()
                        if isinstance(render_output, dict):
                            frame = render_output.get('img', None)
                        else:
                            frame = render_output
                            
                        if frame is not None:
                            self.video_manager.add_frame(frame)
                    except Exception as e:
                        print(f"Warning: Failed to capture frame: {e}")
            
            # 记录episode结果
            episode_success = info.get('success', False)
            rewards.append(episode_reward)
            successes.append(episode_success)
            phase_progresses.append(max_phase)
            
            # 完成视频录制
            if recording_this_episode:
                episode_info = {
                    'success': episode_success,
                    'reward': episode_reward,
                    'length': steps,
                    'max_phase': max_phase,
                    'final_phase': info.get('phase', 0)
                }
                video_path = self.video_manager.finish_episode_recording(episode_info)
                if video_path:
                    print(f"Recorded episode {episode_idx}: {video_path}")
        
        mean_reward = np.mean(rewards)
        success_rate = np.mean(successes)
        avg_phase = np.mean(phase_progresses)
        
        # 输出视频录制摘要
        if should_record:
            summary = self.video_manager.get_evaluation_summary()
            print(f"Video recording summary: Evaluation {summary['evaluation_count']}")
            print(f"Next video recording at evaluation {summary['next_video_eval']}")
        
        return mean_reward, success_rate, avg_phase
    
    def _evaluate(self, n_episodes: int = 10):
        """原始评估函数（无视频录制）"""
        print("Evaluating...")
        rewards = []
        successes = []
        phase_progresses = []
        
        for _ in range(n_episodes):
            obs, info = self.env.reset()
            episode_reward = 0
            done = False
            steps = 0
            max_phase = 0
            
            while not done and steps < self.config['max_episode_steps']:
                # 确定性动作
                action, _, _ = self.agent.get_action(obs, deterministic=True)
                obs, reward, terminated, truncated, info = self.env.step(action[0])
                done = terminated or truncated
                episode_reward += reward
                steps += 1
                max_phase = max(max_phase, info.get('phase', 0))
            
            rewards.append(episode_reward)
            successes.append(info.get('success', False))
            phase_progresses.append(max_phase)
        
        mean_reward = np.mean(rewards)
        success_rate = np.mean(successes)
        avg_phase = np.mean(phase_progresses)
        
        return mean_reward, success_rate, avg_phase
    
    def _save_checkpoint(self, episode: int, global_step: int, final: bool = False):
        """保存检查点"""
        suffix = 'final' if final else f'ep{episode}'
        checkpoint_path = os.path.join(self.checkpoint_dir, f'checkpoint_{suffix}.pt')
        
        self.agent.save(checkpoint_path)
        
        # 保存训练统计
        stats_path = os.path.join(self.checkpoint_dir, f'stats_{suffix}.json')
        stats = {
            'episode': episode,
            'global_step': global_step,
            'episode_rewards': self.episode_rewards,
            'episode_lengths': self.episode_lengths,
            'success_rates': self.success_rates,
            'phase_progress': self.phase_progress,
            'best_reward': self.best_reward,
            'best_success_rate': self.best_success_rate,
            'config': self.config
        }
        
        with open(stats_path, 'w') as f:
            json.dump(stats, f, indent=2)
        
        print(f"Checkpoint saved: {checkpoint_path}")
    
    def _save_best_model(self, episode: int, global_step: int, reward: float, success_rate: float):
        """保存最佳模型"""
        best_path = os.path.join(self.checkpoint_dir, 'best_model.pt')
        self.agent.save(best_path)
        
        # 保存最佳模型信息
        best_info = {
            'episode': episode,
            'global_step': global_step,
            'reward': reward,
            'success_rate': success_rate,
            'timestamp': datetime.now().isoformat()
        }
        
        info_path = os.path.join(self.checkpoint_dir, 'best_model_info.json')
        with open(info_path, 'w') as f:
            json.dump(best_info, f, indent=2)
        
        print(f"New best model saved! Success rate: {success_rate:.3f}, Reward: {reward:.2f}")


def main():
    parser = argparse.ArgumentParser(description='Improved PPO training for robot grasping')
    
    # 环境参数
    parser.add_argument('--robot_type', type=str, default='panda', choices=['panda', 'ur5e'])
    parser.add_argument('--difficulty', type=str, default='easy', choices=['easy', 'medium', 'hard'])
    parser.add_argument('--use_image_obs', action='store_true', help='Use image observations')
    parser.add_argument('--max_episode_steps', type=int, default=800)  # 减少episode长度
    parser.add_argument('--headless', action='store_true', default=True)
    
    # PPO参数 - 优化的设置
    parser.add_argument('--lr_actor', type=float, default=3e-4)  # 恢复正常学习率
    parser.add_argument('--lr_critic', type=float, default=3e-4)
    parser.add_argument('--gamma', type=float, default=0.99)
    parser.add_argument('--gae_lambda', type=float, default=0.95)
    parser.add_argument('--clip_epsilon', type=float, default=0.2)
    parser.add_argument('--entropy_coef', type=float, default=0.05)  # 增加探索
    parser.add_argument('--value_loss_coef', type=float, default=0.5)
    parser.add_argument('--max_grad_norm', type=float, default=0.5)
    parser.add_argument('--target_kl', type=float, default=0.02)  # 放宽KL限制
    parser.add_argument('--n_epochs', type=int, default=8)  # 减少更新轮数
    parser.add_argument('--batch_size', type=int, default=64)
    parser.add_argument('--buffer_size', type=int, default=2048)
    
    # 训练参数
    parser.add_argument('--total_timesteps', type=int, default=500000)  # 减少总时间步
    parser.add_argument('--log_interval', type=int, default=10)
    parser.add_argument('--save_interval', type=int, default=100)
    parser.add_argument('--eval_interval', type=int, default=50)
    parser.add_argument('--device', type=str, default='auto')
    
    # 文件路径
    parser.add_argument('--checkpoint_dir', type=str, default='./checkpoints_improved')
    parser.add_argument('--experiment_name', type=str, default=None)
    
    # wandb参数
    parser.add_argument('--use_wandb', action='store_true', default=True)
    parser.add_argument('--wandb_project', type=str, default='improved_grasp_ppo')
    parser.add_argument('--wandb_entity', type=str, default=None)
    parser.add_argument('--wandb_offline', action='store_true', default=True)
    
    # 视频录制参数
    parser.add_argument('--video_interval', type=int, default=5, 
                       help='Record video every N evaluations')
    parser.add_argument('--max_videos_per_eval', type=int, default=3,
                       help='Maximum videos to record per evaluation')
    parser.add_argument('--video_fps', type=int, default=30,
                       help='Video frame rate')
    parser.add_argument('--disable_video', action='store_true',
                       help='Disable video recording')
    
    args = parser.parse_args()
    
    # 创建配置字典
    config = vars(args)
    
    # 视频配置
    config['video_config'] = {
        'interval': config['video_interval'] if not config['disable_video'] else 999999,  # 禁用时设置很大的值
        'max_per_eval': config['max_videos_per_eval'],
        'fps': config['video_fps']
    }
    
    # 设置实验名称
    if config['experiment_name'] is None:
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        config['experiment_name'] = f"improved_grasp_{config['robot_type']}_{config['difficulty']}_{timestamp}"
    
    # 更新检查点目录
    config['checkpoint_dir'] = os.path.join(config['checkpoint_dir'], config['experiment_name'])
    
    # 初始化wandb
    if config['use_wandb']:
        wandb_mode = "offline" if config['wandb_offline'] else "online"
        print(f"Wandb mode: {wandb_mode}")
        
        wandb.init(
            project=config['wandb_project'],
            entity=config['wandb_entity'],
            name=config['experiment_name'],
            config=config,
            save_code=True,
            mode=wandb_mode
        )
    
    print("Improved Training Configuration:")
    for key, value in config.items():
        print(f"  {key}: {value}")
    
    # 创建训练器并开始训练
    trainer = ImprovedPPOTrainer(config)
    
    try:
        trainer.train()
    except KeyboardInterrupt:
        print("\nTraining interrupted by user")
    except Exception as e:
        print(f"\nTraining failed with error: {e}")
        raise
    finally:
        if config['use_wandb']:
            wandb.finish()


if __name__ == '__main__':
    main()