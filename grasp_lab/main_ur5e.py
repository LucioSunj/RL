...
if __name__ == '__main__':
    ...
    img = imgs['cam_2']
    print("imgs['cam_2']: ", type(img), img.shape, img.dtype)  # 应该是: <class 'numpy.ndarray'> (H, W, 3) uint8
    video_writer = imageio.get_writer("camera_view.mp4", fps=30, format='FFMPEG')
    frame_interval = int(1 / (30 * 0.002))
    print("frame_interval: ", frame_interval)
    i_step = 0
    video_writer.append_data(imgs['cam_2'])

    robot = env.robot
    T_wb = robot.base
    n_wc = np.array([0.0, -1.0, 0.0])
    o_wc = np.array([-1.0, 0.0, -0.3])
    t_wc = np.array([0.0, 0.0, 2.0])
    ...
    time4 = t_gap
    T5 = sm.SE3.Trans(0.6, -0.25, T4.t[2]) * sm.SE3(sm.SO3(T4.R))
    ...
    time6 = t_gap
    T7 = sm.SE3.Trans(0.0, 0.0, -0.1) * T5
    position_parameter6 = LinePositionParameter(T5.t, T7.t)
    attitude_parameter6 = OneAttitudeParameter(sm.SO3(T5.R), sm.SO3(T7.R))
    cartesian_parameter6 = CartesianParameter(position_parameter6, attitude_parameter6)
    velocity_parameter6 = QuinticVelocityParameter(time6)
    trajectory_parameter6 = TrajectoryParameter(cartesian_parameter6, velocity_parameter6)
    planner6 = TrajectoryPlanner(trajectory_parameter6)
    ...
    time_array = [0.0, time3, time4, time6]
    planner_array = [planner3, planner4, planner6]
    total_time = np.sum(time_array)
